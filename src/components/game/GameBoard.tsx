...
export function GameBoard({
  ...
  /** Map a touch point to a grid cell. Returns null when coordinates are
   *  unreliable (no active touch, zero-size bounding box, or out of range) so
   *  NaN/Infinity can never reach placement logic. */
  const touchToCell = useCallback(
    (e: React.TouchEvent): Cell | null => {
  ...
  const handleTouchStart = useCallback(
    (e: React.TouchEvent) => {
      if (isGameOver) return;
      const cell = touchToCell(e);
      if (cell) {
        touchCellRef.current = cell;
        onHoverCell(cell.row, cell.col);
      }
    },
    [isGameOver, onHoverCell, touchToCell],
  );
